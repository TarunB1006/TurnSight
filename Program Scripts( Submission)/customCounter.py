...
class ObjectCounter:
    ...
    def extract_and_process_tracks(self, tracks,frame):
                ...
                if len(self.reg_pts) >= 3:
                    ...
                    if prev_position is not None and is_inside and track_id not in self.count_ids:
                        if track_id not in hashmap:
                            hashmap.update({track_id:self})
                            self.count_ids.update({track_id:[cls,'source']})
                        else:
                            previousCounter=hashmap[track_id]
                            if(previousCounter.count_ids[track_id][1]=='dest'):
                                del previousCounter.count_ids[track_id]
                            self.count_ids.update({track_id:[cls,'dest',frame]})
                            hashmap.update({track_id:self})
    ...
    def start_counting(self, im0, tracks,frame):
        """
        ...
        self.extract_and_process_tracks(tracks,frame)  # draw region even if no objects
...
if __name__ == "__main__":
    classes_names = {0:"car"}  # dummy testing
